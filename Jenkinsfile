pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-Interactive-Pricing-Component-Deployment.git'
            }
        }
        ...
        stage("Sonarqube Analysis ") {                         
            steps {
                //dir('Band Website') {
                    ...
                    sh '''$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=ipc-application \
                    -Dsonar.projectKey=ipc-application'''
                    //}
                }
            }
        }
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag ipcomponent yash5090/ipcomponent:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/ipcomponent:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/ipcomponent:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/ipcomponent:latest'
                       sh 'docker-scout cves yash5090/ipcomponent:latest'
                       sh 'docker-scout recommendations yash5090/ipcomponent:latest'
                       sh 'docker-scout attestation yash5090/ipcomponent:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/ipcomponent:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name ipcomponent -p 3000:3000 yash5090/ipcomponent:latest' 
                //}
            }
        }
